...
class dddQ_Net(nn.Module):
	def __init__(self, obs_dim, action_dim):
		...
		self.A = nn.Linear(100, action_dim)  # the advantage of actions, relative value of each action
...
class dddQN_Agent(object):
	def __init__(self, obs_dim, agent_name, lrate, tlrate, gamma, batch_size, action_dim, target_freq, hardtarget, exp_noise, transfer_train):
		self.q_net = dddQ_Net(obs_dim, action_dim).to(device)
	...
	def select_action(self, state, action_mask, evaluate):
		with torch.no_grad():
			...
			if np.random.rand() < epsilon:
				...
				V, A, Q = self.q_net.forward(state.float())
	...
	def train(self, replay_buffer,huber_loss):
		...
		if huber_loss: q_loss = F.huber_loss(current_q_a, target_Q)
	...
	def save(self,algo,EnvName):
		save_path = "..\\ddDQN\\model\\{}_{}.pth".format(algo, EnvName)
	...
	def load(self, algo, EnvName, input_obs_dim, conv_input_dim, new_action_dim, old_action_dim, lrate, transfer_train=False):
		self.q_net.load_state_dict(torch.load("..\\ddDQN\\model\\{}_{}.pth".format(algo,EnvName)))
		self.q_target.load_state_dict(torch.load("..\\ddDQN\\model\\{}_{}.pth".format(algo,EnvName)))
		if transfer_train:
	...
	def coarse_tuning_settings(self):
		...
		for param in self.q_net.get_conv_layers().parameters():
