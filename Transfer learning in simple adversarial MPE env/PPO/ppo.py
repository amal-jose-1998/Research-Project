...
class ActorNetwork(nn.Module):
    def __init__(self, n_actions, input_dims, alpha, EnvName, fc1_dims=256, fc2_dims=256, algo='PPO'):
        super(ActorNetwork, self).__init__()
        self.save_path = "./model/{}_{}.pth".format(algo, EnvName)
        self.conv_layers = nn.Sequential(                                           # input tensor shape = (batch_size, channels, height, width)
				nn.Conv2d(in_channels=input_dims[2], out_channels=5, kernel_size=3, stride=1),           
				nn.ReLU(),
				nn.Conv2d(in_channels=5, out_channels=10, kernel_size=3, stride=1),            
				nn.ReLU(),
				nn.Conv2d(in_channels=10, out_channels=15, kernel_size=3, stride=1, padding=1),
				nn.ReLU(),
				...
				nn.ReLU(),
				nn.Conv2d(in_channels=25, out_channels=30, kernel_size=3, stride=1),   # output length = (input length + 2*padding - kernal)/stride  +1
				nn.ReLU(),
				nn.Flatten())
        self.conv_out_dim = (input_dims[0]-6)*(input_dims[1]-6)*30
    ...
    def save_checkpoint(self):
        os.makedirs(os.path.dirname(self.save_path), exist_ok=True)  # Create the directory if it doesn't exist
...
class CriticNetwork(nn.Module):
    def __init__(self, input_dims, alpha, EnvName, fc1_dims=256, fc2_dims=256, algo='PPO'):
...
class Agent:
    def __init__(self, n_actions, input_dims, gamma, alpha, gae_lambda, policy_clip, batch_size, n_epochs, env_name, agent_name):
        ...
        self.actor = ActorNetwork(n_actions, input_dims, alpha, f'{env_name}_{agent_name}_actor')
        self.critic = CriticNetwork(input_dims, alpha, f'{env_name}_{agent_name}_critic')
    ...
    def choose_action(self, observation):
        ...
        if masked_dist.sum() == 0:
            action = dist.probs.sample() # sample an action
        else:
        ...
        probability = T.squeeze(dist.log_prob(action)).item()  # log probability of the chosen action from the distribution
