...
def train_from_scratch(agent_models, agent_buffers, num_games, env, eval_env, n_good_agents):
    ...
    for agent_id in range(n_good_agents+1):  
        ...
        agent_obs_dim, agent_action_dim = set_observation_dimension(agent_id, pretrain = agent_opt.pretrain)
        model = dddQN_Agent(agent_obs_dim, agent_id, agent_opt.lr, agent_opt.gamma, agent_opt.batch_size, agent_action_dim, agent_opt.target_freq, agent_opt.hardtarget, agent_opt.exp_noise, agent_opt.transfer_train)  # Create a model for each agent    
