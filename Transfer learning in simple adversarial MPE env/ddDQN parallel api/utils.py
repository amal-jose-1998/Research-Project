...
def loop_iteration(num_games, env, eval_env, opt, agent_models, agent_buffers, good_agents, epochs): 
                ...
                if any(terminations.values()) or any(truncations.values()):
                    ...
                    for agent_name in env.agents:
                        ...
                        if buffer.size >= opt.random_steps: #checks if the replay buffer has accumulated enough experiences to start training.
                            ...
                            if total_steps % opt.train_freq == 0: 
                                model = agent_models[agent_id]
                                loss[agent_id] = model.train(buffer)
                                if opt.write:
                                    wandb.log({f'training Loss for agent{agent_id}': loss[agent_id].item()})
                                model.exp_noise = schedualer.value(total_training_steps) #e-greedy decay
                                print('episode: ',i,'training step: ',total_training_steps,'loss of agent ',agent_id,': ',loss[agent_id].item())
                        agent_id+=1
                    ...
                    if flag:
                        ...
                        if total_training_steps % opt.eval_interval == 0:
                            ...
                            if opt.write:
                                wandb.log({'evaluation_env  avg_reward': score, 'total steps': total_steps, 'episode': i, 'epoch': _})
                                print("Evaluation")
