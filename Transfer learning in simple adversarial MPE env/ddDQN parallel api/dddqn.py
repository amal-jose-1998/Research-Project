...
class dddQN_Agent(object):
	def __init__(self, obs_dim, id, lrate, gamma, batch_size, action_dim, target_freq, hardtarget, exp_noise, transfer_train):
		self.q_net = dddQ_Net(obs_dim, id).to(device)
	...
	def load(self, algo, EnvName, input_obs_dim, conv_input_dim, new_action_dim, old_action_dim, lrate, transfer_train=False):
		...
		print("source network for: ", algo)
		summary(self.q_net, input_size=(1,conv_input_dim))
		if transfer_train:
