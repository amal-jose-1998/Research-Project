...
parser.add_argument('--random_steps', type=int, default=1000, help=' min no of replay buffer experiences to start training')
...
parser.add_argument('--games', type=int, default=10, help='no of games to play')
        ...
        while not done:
            if terminations or truncations:
                ...
                for agent_name in env.agents:
                    ...
                    r[agent_name]=torch.tensor(r[agent_name])
                    actions[agent_name]=torch.tensor(actions[agent_name])
                    if terminations[agent_name] or truncations[agent_name]:# should the done flag be made true if any one agent is killed
                        done = True
                        done = torch.tensor(done)
                    buffer = agent_buffers[i]
                    buffer.add(s[agent_name], actions[agent_name], r[agent_name], s_prime[agent_name], done) # should the done flag be converted to tensor
                    s[agent_name] = s_prime[agent_name]
                    i+=1
                    print(buffer.size)
